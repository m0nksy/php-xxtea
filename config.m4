...
dnl PHP_ARG_ENABLE(xxtea, whether to enable xxtea support,
dnl Make sure that the comment is aligned:
dnl [  --enable-xxtea           Enable xxtea support])
...
if test "$PHP_XXTEA" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $XXTEA_DIR/$PHP_LIBDIR, XXTEA_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_XXTEALIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong xxtea lib version or lib not found])
  dnl ],[
  dnl   -L$XXTEA_DIR/$PHP_LIBDIR -lm
  dnl ])
  dnl
  dnl PHP_SUBST(XXTEA_SHARED_LIBADD)

  PHP_NEW_EXTENSION(xxtea, xxtea.c libxxtea.c, $ext_shared)
fi
